...
const BestSeller = () => {
  ...
  const getBestSellerProducts = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(
          response.data.message || "Failed to fetch best seller products."
        );
        console.log(
  ...
  useEffect(() => {
    on(SOCKET_EVENTS.PRODUCT_BESTSELLER_UPDATED, (data) => {
      toast.success("Best seller products updated!");
      getBestSellerProducts(); // refetch via API
    });

    return () => off(SOCKET_EVENTS.PRODUCT_BESTSELLER_UPDATED);
  }, []);
    ...
    <div className="my-10">
      <div className="text-center py-8 text-3xl">
        ...
        <p className="w-3/4 m-auto text-xs sm:text-sm md:text-base text-gray-600">
          Discover our most-loved fashion picks — the perfect blend of comfort,
          style, and quality. From everyday essentials to trend-setting designs,
          shop the pieces everyone’s talking about.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 gap-y-6 px-4">
        {loading
          ? Array(6)
              .fill()
              .map((_, idx) => <ShimmerCard key={idx} />) // Use ShimmerCard for loading state
